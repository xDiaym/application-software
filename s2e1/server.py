...
class SQLiteStorage:
    ...
    async def store_message(self, author: str, text: str) -> None:
        author_id: int
        async with self._connection.execute("SELECT id FROM users WHERE nick = ?", (author, )) as cursor:
            if author_id := await cursor.fetchone() is None:
                logger.warning("user %s not found", author)
                return

        query = "INSERT INTO messages(author_id, channel_id, text_) VALUES (?, ?, ?)"
        async with self._connection.execute(query, (author_id, GLOBAL_CHAT_ID, text)):
            await self._connection.commit()

    async def get_by_date_period(self, author: str, start: datetime.datetime, end: datetime.datetime) -> None:
        raise NotImplementedError
    
    async def register(self, nick: str, password: str) -> bool:
        async with self._connection.execute(
            "SELECT COUNT(*) FROM users WHERE nick = ?",
            (nick, )
        ) as cursor:
